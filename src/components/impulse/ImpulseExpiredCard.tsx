  ...
  return (
    <Card>
      <div className="flex flex-col gap-3">
        <div>
          <p className="text-[10px] font-medium uppercase tracking-[0.14em] text-ink-4 mb-2">冷静期结束</p>
          <p className="text-base font-medium text-ink">{record.item_name}</p>
          <p className="mt-0.5 text-xs text-ink-4">
            {since}种草
            {record.estimated_price != null && <span> · 约 <span className="font-serif">{formatAmount(record.estimated_price)}</span></span>}
            {record.source && <span> · {record.source}</span>}
          </p>
        </div>
        <p className="text-sm text-ink-3">冷静期结束了，你还想要吗？</p>
