...
function snapshotFacts(s: MonthlyStorySnapshot): string {
  ...
  if (s.persona) {
    lines.push(`本月消费人格（系统判定，可引用）：${s.persona.emoji} ${s.persona.title} —— ${s.persona.description}`)
  }
  return lines.join('\n')
}
...
export function buildStoryPrompt(snapshot: MonthlyStorySnapshot): AIMessage[] {
  const system =
    ...
    '3. 条件段落：只有数据里有时才写——后悔的消费（可引用「几乎没怎么用」这类使用情况）、忍住次数与许愿池进度、冲动过滤、消费人格。没有就整段跳过，不要写「没有后悔」这类填充。\n' +
    '4. 结尾永远只用一句话收束，克制、略带自嘲、不说教、不喊口号。\n' +
    '5. 语气像写给自己看的随手记，短句分行，不用列表符号，不用 Markdown，不输出 JSON。总长控制在 6 段以内。\n' +
    '6. 金额直接用数据里的数字，格式如 ¥3240。\n\n' +
    `风格参考（只学语气和节奏，不要照抄内容）：\n${STYLE_EXAMPLE}`
