  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex w-fit items-center gap-1.5 rounded-lg border-theme bg-card-alt px-2.5 py-1 text-xs text-ink-2 transition-colors hover:bg-[var(--bg-card)]"
      >
        ...
        <div className="rounded-[12px] border-theme bg-card p-3 shadow-sm">
          {CATEGORY_GROUPS.map((group) => (
            <div key={group.main} className="mb-3 last:mb-0">
              <p className="mb-2 px-1 text-[9px] font-medium uppercase tracking-[0.14em] text-ink-4">
                {group.label}
              </p>
